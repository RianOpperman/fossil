...
BOOT_OBJ := $(patsubst %.s, %.o, $(BOOT_FILE))
SRC_FILES := $(shell find $(PROJ_DIRS) -type f -name "*.c") # Matches ASM & C files
HDR_FILES := $(shell find $(PROJ_DIRS) -type f -name "*.h")
C_OBJ_FILES := $(patsubst %.c,%.o,$(SRC_FILES))
OBJ_FILES := $(C_OBJ_FILES) $(BOOT_OBJ)
...
all: $(BOOT_OBJ) $(OBJ_FILES) fossil.bin

# Source files & Compilation
$(BOOT_OBJ): $(BOOT_FILE)
	@$(AS) $(BOOT_FILE) -o $(BOOT_OBJ)
